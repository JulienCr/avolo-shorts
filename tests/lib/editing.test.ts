...
describe('isWordKept', () => {
  it('un mot dans un segment est monté', () => {
    expect(isWordKept(words[1], [{ start: 10, end: 15 }])).toBe(true)
  })

  it('un mot hors de tout segment est barré', () => {
    expect(isWordKept(words[1], [{ start: 12, end: 15 }])).toBe(false)
  })
  ...
  it('un mot à cheval sur une borne est monté : on l’entendra', () => {
    ...
    expect(isWordKept(words[1], [{ start: 11.4, end: 15 }])).toBe(true)
  })

  it('sans segment, plus rien n’est monté', () => {
    expect(isWordKept(words[0], [])).toBe(false)
  })
})

describe('indexTranscript', () => {
  const lines: TranscriptLine[] = [
    { id: 'a', start: 10, end: 11.8, words: words.slice(0, 2) },
    { id: 'b', start: 12, end: 14.8, words: words.slice(2) },
  ]

  it('numérote les mots à plat, phrases confondues', () => {
    const { words } = indexTranscript(lines, [])
  ...
  it('marque les mots retirés', () => {
    const { words } = indexTranscript(lines, [{ start: 10, end: 12.9 }])
...
describe('selectionBounds', () => {
  it('va du début du premier mot à la fin du dernier', () => {
    expect(selectionBounds(words, 1, 3)).toEqual({ from: 11, to: 13.8 })
  })
  ...
  it('accepte une sélection faite à l’envers', () => {
    ...
    expect(selectionBounds(words, 3, 1)).toEqual(selectionBounds(words, 1, 3))
  })

  it('un seul mot se sélectionne', () => {
    expect(selectionBounds(words, 2, 2)).toEqual({ from: 12, to: 12.8 })
  })

  it('rend null hors bornes plutôt que de lever', () => {
    expect(selectionBounds(words, -1, 2)).toBeNull()
    expect(selectionBounds(words, 2, 99)).toBeNull()
...
describe('removeSelection', () => {
  ...
  it('coupe le clip en deux quand on retire son milieu', () => {
    expect(removeSelection(segments, words, 2, 2)).toEqual([
      { start: 10, end: 12 },
      { start: 12.8, end: 15 },
    ])
  })
  ...
  it('la durée baisse d’exactement la sélection retirée', () => {
    const after = removeSelection(segments, words, 1, 3)
    expect(clipDuration(segments) - clipDuration(after)).toBeCloseTo(2.8, 6)
  })

  it('une sélection impossible ne change rien', () => {
    expect(removeSelection(segments, words, 0, 99)).toEqual(segments)
  })

  it('retirer deux fois de suite est idempotent', () => {
    const a = removeSelection(segments, words, 2, 2)
    expect(removeSelection(a, words, 2, 2)).toEqual(a)
  })
})
...
describe('restoreBounds', () => {
  it('remonte du mot précédent au mot suivant, pas seulement le mot', () => {
    ...
    // demandé.
    expect(restoreBounds(words, 2)).toEqual({ from: 11.8, to: 13 })
  })

  it('s’arrête au mot lui-même aux extrémités de la liste', () => {
    expect(restoreBounds(words, 0)).toEqual({ from: 10, to: 11 })
    expect(restoreBounds(words, 4)).toEqual({ from: 13.8, to: 14.8 })
  })
  ...
  it('supporte des mots qui se chevauchent, comme en parole rapide', () => {
    ...
    expect(restoreBounds(fast, 1)).toEqual({ from: 1, to: 2.2 })
  })
  ...
  it('ne rejoint pas non plus vers l’avant par-dessus un long silence', () => {
    ...
    expect(restoreBounds(separated, 1)).toEqual({ from: 10.5, to: 11.2 })
  })
  ...
  it('rejoint encore par-dessus une respiration entre deux phrases', () => {
    ...
    expect(restoreBounds(breath, 1)).toEqual({ from: 10.5, to: 11.9 })
  })

  it('rend null hors bornes', () => {
    expect(restoreBounds(words, 5)).toBeNull()
...
describe('restoreWord', () => {
  it('rend un mot barré et le recolle au segment voisin, sans trou', () => {
    ...
    expect(restoreWord(segments, words, 2)).toEqual([{ start: 10, end: 15 }])
  })
  ...
  it('deux mots rendus l’un après l’autre finissent par fusionner', () => {
    ...
    const afterA = restoreWord(segments, words, 1)
    const afterTwo = restoreWord(afterA, words, 2)
    expect(afterTwo).toEqual([{ start: 10, end: 15 }])
  })

  it('un index impossible ne change rien', () => {
    const segments: Segment[] = [{ start: 10, end: 11 }]
    expect(restoreWord(segments, words, 42)).toEqual(segments)
  })
})
...
describe('moveBoundaryToWord', () => {
  ...
  it('pose la borne de début au début du mot', () => {
    expect(moveBoundaryToWord(segments, words, 0, 'start')).toEqual([{ start: 10, end: 13 }])
  })

  it('pose la borne de fin à la fin du mot', () => {
    expect(moveBoundaryToWord(segments, words, 4, 'end')).toEqual([{ start: 11, end: 14.8 }])
  })
  ...
  it('rétrécir traverse les segments qu’il faut', () => {
    ...
    expect(moveBoundaryToWord(two, words, 3, 'start')).toEqual([{ start: 13, end: 14.8 }])
  })
})
...
describe('clipBounds', () => {
  it('relit les bornes dans la liste rendue, jamais la valeur demandée', () => {
    ...
    const after = moveBoundaryToWord(two, words, 2, 'start')
    expect(clipBounds(after)).toEqual({ start: 13, end: 14.8 })
    expect(clipBounds(after)?.start).not.toBe(words[2].start)
  })
...
describe('ligneInitiale', () => {
  ...
  it('ouvre sur la phrase où le clip commence, pas sur le contexte d’avant', () => {
    expect(lineInitial(lines, [{ start: 11, end: 14 }])).toBe(2)
  })

  it('prend la phrase qui contient le début, même s’il tombe en son milieu', () => {
    expect(lineInitial(lines, [{ start: 7.5, end: 9 }])).toBe(1)
  })
  ...
  it('ouvre en haut quand le clip n’a plus de segment', () => {
    ...
    expect(lineInitial(lines, [])).toBe(0)
  })

  it('ouvre en haut quand aucune phrase ne va jusque-là', () => {
    expect(lineInitial(lines, [{ start: 99, end: 100 }])).toBe(0)
  })
})
...
describe('applyWordCorrection', () => {
  it('remplace un mot par un autre, sans toucher aux voisins', () => {
    const result = applyWordCorrection(words, {
      from: 2,
      to: 2,
      expected: ['trois'],
      replacement: ['3'],
    ...
    expect(result).toEqual({
      ok: true,
      words: [words[0], words[1], { word: '3', start: 12, end: 12.8 }, words[3], words[4]],
    })
  })
  ...
  it('fusionne deux mots en un, qui prend leur empan réuni', () => {
    ...
    expect(result.words.slice(1)).toEqual([words[2], words[3], words[4]])
  })
  ...
  it('supprime un mot — un remplacement vide', () => {
    ...
    expect(result).toEqual({ ok: true, words: [words[0], words[2], words[3], words[4]] })
  })
  ...
  it('refuse un empan qui déborde de la phrase', () => {
    expect(
      applyWordCorrection(words, { from: 0, to: 10, expected: [], replacement: ['x'] }),
    ).toEqual({ ok: false, reason: 'out-of-range' })
  })

  it('refuse des bornes inversées', () => {
    expect(
      applyWordCorrection(words, { from: 3, to: 1, expected: [], replacement: ['x'] }),
    ).toEqual({ ok: false, reason: 'out-of-range' })
  })

  it('ne modifie pas le tableau reçu', () => {
    const copy = words.map((m) => ({ ...m }))
    applyWordCorrection(words, { from: 0, to: 0, expected: ['un'], replacement: ['1'] })
