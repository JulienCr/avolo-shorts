...
describe('gestureOnWordBar', () => {
  it('comble le trou quand le mot est dans l’étendue', () => {
    expect(gestureOnWordBar(extent, { start: 140, end: 140.4 })).toEqual({ kind: 'restore' })
  })
  ...
  it('comble le trou quand le mot chevauche une borne', () => {
    ...
    expect(gestureOnWordBar(extent, { start: 99.8, end: 100.2 })).toEqual({ kind: 'restore' })
    expect(gestureOnWordBar(extent, { start: 199.8, end: 200.2 })).toEqual({ kind: 'restore' })
  })
  ...
  it('remonte, faute d’étendue, quand tous les mots ont été retirés', () => {
    ...
    expect(gestureOnWordBar(null, { start: 40, end: 40.3 })).toEqual({ kind: 'restore' })
  })
})
