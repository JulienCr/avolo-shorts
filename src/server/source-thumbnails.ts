...
export function folderVignettesSources(): string {
 ...
 * le fichier écrit.
 */
export function sourceVerifyName(name: string): string {
  const rejected =
    name === '' ||
    name === '.' ||
    name === '..' ||
    ...
    name.includes('\\') ||
    name.includes('\0')
  if (rejected) throw new SourceInvalidError(`Nom de source invalide : ${JSON.stringify(name)}`)
  return name
}
 ...
 */
const NAME_READABLE_OCTETS = 96
const FINGERPRINT_HEX = 12

function cacheName(name: string, key: string): string {
  ...
  const readable = Buffer.from(name, 'utf8')
    .subarray(0, NAME_READABLE_OCTETS)
    .toString('utf8')
    .replace(/\uFFFD+$/, '')
  const fingerprint = createHash('sha256').update(name, 'utf8').digest('hex').slice(0, FINGERPRINT_HEX)
  return `${readable}.${fingerprint}.${key}.jpg`
}
 ...
 */
export function vignetteSourcePath(name: string, sizeBytes: number, mtimeMs: number): string {
  sourceVerifyName(name)
  return path.join(
    folderVignettesSources(),
    cacheName(name, `${sizeBytes}-${Math.trunc(mtimeMs)}`),
  )
}
 ...
 */
export function urlVignetteSource(name: string, sizeBytes: number, mtimeMs: number): string {
  const version = `${sizeBytes}-${Math.trunc(mtimeMs)}`
  return `/api/sources/thumb?file=${encodeURIComponent(name)}&v=${version}`
}
 ...
 */
export const FALLBACK_MOMENT_S = 300

export function momentVignetteSource(durationSec: number | null): number {
  if (durationSec === null || !Number.isFinite(durationSec) || durationSec <= 0) {
    return FALLBACK_MOMENT_S
  }
...
export type OptionsVignetteSource = {
  ...
  probe?: (file: string, timeoutMs: number) => Promise<number | null>
  /** L'extraction elle-même. Injectée par les tests, qui n'ont pas de ffmpeg. */
  extract?: (o: { src: string; dst: string; at: number }, timeoutMs: number) => Promise<void>
}
 ...
 * part.
 */
const GIVE_UP = 'vignettes:délai-dépassé'

/** Ce que l'appelant voit quand le montage n'a pas répondu. */
export class EditingMuteError extends Error {
  constructor() {
    super(
 ...
 * plus haut.
 */
let inDistress: Promise<unknown> | null = null

/** Retient l'accès abandonné jusqu'à ce qu'il se règle enfin. */
function keep(raw: Promise<unknown>): void {
  inDistress = raw
  ...
  void raw
    .finally(() => {
      if (inDistress === raw) inDistress = null
    })
    .catch(() => {})
}

/** Rouvre le passage. La sortie explicite, et ce dont les tests se servent. */
export function rearmCircuitBreaker(): void {
  inDistress = null
}
 ...
 */
async function underGuard<T>(work: () => Promise<T>, timeoutMs: number): Promise<T> {
  ...
  if (inDistress !== null) throw new EditingMuteError()
  const raw = work()
  try {
    return await waitOrAbandon(raw, timeoutMs, GIVE_UP)
  } catch (cause) {
    if (cause instanceof Error && cause.message === GIVE_UP) {
      keep(raw)
      throw new EditingMuteError()
    }
    throw cause
  }
}
...
function inFile<T>(work: () => Promise<T>): Promise<T> {
  const result = file.then(work, work)
  file = result.then(
    () => {},
    () => {},
  )
...
export async function vignetteSource(
  ...
  const timeoutMs = options.timeoutMs ?? DELAY_STAT_MS

  return inFile(async () => {
    const info = await capture(source, timeoutMs)
    ...
    return produce(source, vignetteSourcePath(name, info.size, info.mtimeMs), timeoutMs, options)
  })
}
...
function resolve(name: string): string {
  try {
    return resolveSource(sourceVerifyName(name))
  } catch (cause) {
    if (cause instanceof SourceInvalidError) throw cause
    ...
    throw new SourceInvalidError(cause instanceof Error ? cause.message : String(cause), {
      cause,
    })
  }
}
...
async function produce(
  ...
  const probe = options.probe ?? probeDuration
  const extract = options.extract ?? extractWithFfmpeg
  ...
  const at = momentVignetteSource(
    await underGuard(() => probe(source, timeoutMs), timeoutMs * MARGIN_CLOSING),
  )

  try {
    await extract({ src: source, dst: temporary, at }, timeoutMs)
