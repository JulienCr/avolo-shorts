 ...
 * et l'appel réel (`createCallFromSettings`) posent exactement la même
 * question sur la même variable.
 */
export function apiKeyVariableFor(provider: LlmProvider): string | undefined {
  switch (provider) {
    case 'gemini':
 ...
 */
export function providerAvailability(provider: LlmProvider): LlmProviderAvailability {
 ...
 */
export function createCall(provider: LlmProvider, options: LlmClientOptions): LlmCall {
  switch (provider) {
    case 'gemini':
      return createGeminiCall(options)
    case 'openai':
      return createOpenAiCall(options)
    case 'ollama':
      return createOllamaCall(options)
  }
}
 ...
 */
export function createCallFromSettings(
  ...
  const { provider, model } = providerModelFor(ai, usage)
  return createCall(provider, {
    model,
