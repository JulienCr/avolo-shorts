...
import type { CauseUnavailable, SourcesListing } from '@/lib/api'
 ...
 */
export function EditingLine({
  editing,
  onRetry,
}: {
  editing: SourcesListing['editing']
  onRetry: () => void
}) {
  const { title, detail, icon, grave } = diagnostic(editing)
 ...
 */
const EDITING_EXPECTED = '9p'

/** Le geste qui répare un partage tombé — il vient de `CLAUDE.md`. */
const REPAIR = 'Rouvrir le lecteur côté Windows, ou remonter le partage.'
 ...
 */
function perCause(cause: CauseUnavailable, fstype: string | null): {
  title: string
  detail: string
} {
  switch (cause) {
    case 'absent':
      return {
        title: 'Le dossier des replays n’existe pas à ce chemin.',
        detail:
          fstype === EDITING_EXPECTED
            ? `Le partage ${EDITING_EXPECTED} répond : c’est donc le chemin qui est faux. Vérifier REPLAY_DIR.`
            : `${reading(fstype)} Vérifier REPLAY_DIR, puis le partage : ${lower(REPAIR)}`,
      }
    case 'denied':
      return {
    ...
    case 'silent':
      return {
        title: 'Le dossier des replays ne répond pas.',
        detail: `Le partage est monté mais n’a rien rendu dans le temps imparti : son transport est mort dessous, et /proc/mounts ne le distingue pas d’un partage sain. ${REPAIR}`,
      }
    case 'unreadable':
      return {
        title: 'Le dossier des replays n’a pas pu être lu.',
        detail: `Le système de fichiers a rendu une erreur que le serveur ne sait pas nommer. ${reading(fstype)} ${REPAIR}`,
      }
  }
}

function diagnostic(editing: SourcesListing['editing']) {
  if (editing.cause !== null) {
    return {
      grave: true,
      icon: <TriangleAlert aria-hidden />,
      ...perCause(editing.cause, editing.fstype),
    }
  }
 ...
 */
function reading(fstype: string | null): string {
  if (fstype === EDITING_EXPECTED) return `Le partage ${EDITING_EXPECTED} répond.`
  if (fstype === null) {
    return `Aucun montage relevé ne porte ce chemin : le partage ${EDITING_EXPECTED} attendu n’est pas là.`
  }
  return `Système de fichiers relevé : ${fstype} — le partage ${EDITING_EXPECTED} attendu n’est pas là.`
}
