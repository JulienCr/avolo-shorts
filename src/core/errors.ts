...
function cleanReferenceBare(raw: string): string {
  return `${REFERENCE_PREFIX}…${raw.match(PUNCTUATION_FINAL)?.[0] ?? ''}`
}
 ...
 */
const KEY_IN_URL = /([?&](?:key|api_?key)=)[^&\s"']+/gi
...
export function redactKeys(message: string): string {
  return message.replace(KEY_IN_URL, '$1[caviardé]')
}
 ...
 */
export function cleanPaths(message: string, roots: readonly string[] = []): string {
  let output = message
  ...
  for (const root of [...roots].filter((r) => r !== '').sort((a, b) => b.length - a.length)) {
  ...
  return redactKeys(output)
    ...
    .replace(
      REFERENCE_CITED,
      (_, delimiter: string) => `${delimiter}${REFERENCE_PREFIX}…${delimiter}`,
    )
    .replace(REFERENCE_BARE, cleanReferenceBare)
    .replace(POSIX_BARE, shorten)
    .replace(WINDOWS_BARE, shorten)
}
 ...
 */
export function messageCleaned(error: unknown, roots: readonly string[] = []): string {
  return cleanPaths(error instanceof Error ? error.message : String(error), roots)
}
