...
import { formatDateSource, formatOctets, messageServer, plural } from '@/components/sources/texts'
...
describe('pluriel', () => {
  it('garde le singulier à zéro, comme le veut le français', () => {
    expect(plural(0, 'replay', 'replays')).toBe('0 replay')
    expect(plural(1, 'replay', 'replays')).toBe('1 replay')
    expect(plural(2, 'replay', 'replays')).toBe('2 replays')
  })
})
...
describe('messageServeur', () => {
  it('reprend le message du serveur tel quel', () => {
    ...
    const server = new ApiError(503, 'Le dossier des replays ne répond pas.')
    expect(messageServer(server)).toBe('Le dossier des replays ne répond pas.')
  })
  ...
  it('ne laisse jamais une exception se lire à voix haute', () => {
    ...
    expect(messageServer(new TypeError('Failed to fetch'))).toBe(
      'Le serveur n’a pas répondu.',
    )
    expect(messageServer('quelque chose')).toBe('Le serveur n’a pas répondu.')
  })
})
