...
import { agreement, idsForView, detectionWord, viewSinceUrl } from '@/components/review/template'
...
describe('idsForView', () => {
  ...
  it('ne garde à trier que ce qui n’est pas décidé', () => {
    expect(idsForView(clips, 'atrier')).toEqual(['a'])
    expect(idsForView(clips, 'ecartes')).toEqual(['c'])
  })
  ...
  it('rend la liste dans l’ordre reçu', () => {
    ...
    expect(idsForView([...clips].reverse(), 'gardes')).toEqual(['d', 'b'])
  })
})
...
describe('viewSinceUrl', () => {
  it('retombe sur « à trier » pour tout ce qu’elle ne connaît pas', () => {
    ...
    expect(viewSinceUrl(null)).toBe('atrier')
    expect(viewSinceUrl('')).toBe('atrier')
    expect(viewSinceUrl('n’importe quoi')).toBe('atrier')
  })

  it('reconnaît les trois vues', () => {
    expect(viewSinceUrl('atrier')).toBe('atrier')
    expect(viewSinceUrl('gardes')).toBe('gardes')
    expect(viewSinceUrl('ecartes')).toBe('ecartes')
  })
})

function summary(fields: Partial<SelectionReport> = {}): SelectionReport {
  return {
    windows: 83,
    scored: 83,
    rejectedBatches: 0,
...
describe('detectionWord', () => {
  it('ne dit rien quand rien n’a été mesuré', () => {
    ...
    // se taire.
    expect(detectionWord(null)).toBeNull()
  })
  ...
  it('dit la couverture, pas les lots, quand une part n’a pas été jugée', () => {
    ...
    const word = detectionWord(summary({ scored: 57, rejectedBatches: 4, answeredBatches: 7, coverage: 0.684 }))
    expect(word?.loss).toBe(true)
    expect(word?.phrase).toContain('68 %')
    expect(word?.phrase).toContain('57 fenêtres sur 83')
    expect(word?.detail).toContain('4 lots')
    expect(word?.detail).toContain('11')
  })
  ...
  it('arrondit la couverture vers le bas tant qu’il manque quelque chose', () => {
    ...
    const word = detectionWord(summary({ scored: 82, rejectedBatches: 1, answeredBatches: 10, coverage: 0.996 }))
    expect(word?.phrase).toContain('99 %')
  })
  ...
  it('ne promet aucun recours', () => {
    ...
    const word = detectionWord(summary({ scored: 57, rejectedBatches: 4, answeredBatches: 7, coverage: 0.684 }))
    expect(`${word?.phrase} ${word?.detail}`).not.toMatch(/relanc|réessay|recommenc/i)
  })
  ...
  it('ne parle pas de refus quand il n’y en a pas eu', () => {
    ...
    const word = detectionWord(summary({ scored: 40, rejectedBatches: 0, answeredBatches: 5, coverage: 0.48, partial: true }))
    expect(word?.loss).toBe(true)
    expect(word?.provisional).toBe(true)
  ...
  it('dit aussi quand tout a été jugé', () => {
    ...
    expect(word?.loss).toBe(false)
    expect(word?.phrase).toContain('83')
    expect(word?.detail).toBeNull()
  })
  ...
  it('n’annonce pas de perte quand la récupération a tout rattrapé', () => {
    ...
    const message = detectionWord(summary({ scored: 83, rejectedBatches: 4, answeredBatches: 11 }))
    expect(message?.loss).toBe(false)
  ...
  it('tient les deux bornes de la couverture', () => {
    ...
    const nothing = detectionWord(summary({ scored: 0, rejectedBatches: 11, answeredBatches: 0, coverage: 0 }))
    expect(nothing?.phrase).toContain('0 %')
    expect(nothing?.loss).toBe(true)
    ...
    // perte.
    const covered = detectionWord(summary({ scored: 82, rejectedBatches: 1, answeredBatches: 10, coverage: 1 }))
    expect(covered?.phrase).toContain('100 %')
    expect(covered?.loss).toBe(true)
    expect(covered?.detail).toContain('1 lot de fenêtres sur 11')
  })

  it('ne rapporte rien à zéro fenêtre', () => {
    // Un transcript vide : il n'y avait rien à noter, donc aucun pourcentage
    // n'a de dénominateur.
    const word = detectionWord(summary({ windows: 0, scored: 0, answeredBatches: 0, coverage: 0 }))
    expect(word?.loss).toBe(false)
    expect(word?.phrase).not.toContain('%')
  })

  it('accorde le singulier', () => {
    expect(detectionWord(summary({ windows: 1, scored: 1, answeredBatches: 1 }))?.phrase).toContain(
      'la fenêtre',
    )
    const a = detectionWord(summary({ scored: 82, rejectedBatches: 1, answeredBatches: 10, coverage: 0.9 }))
    expect(a?.detail).toContain('1 lot de fenêtres sur 11 a été refusé')
  })
})
...
describe('accord', () => {
  it('met le singulier à zéro et à un', () => {
    ...
    expect(agreement(0, 'clip gardé', 'clips gardés')).toBe('0 clip gardé')
    expect(agreement(1, 'clip gardé', 'clips gardés')).toBe('1 clip gardé')
    expect(agreement(2, 'clip gardé', 'clips gardés')).toBe('2 clips gardés')
  })
})
