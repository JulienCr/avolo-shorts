...
describe('messageSafe, sur les références de secret', () => {
  it('caviarde une référence à espaces, hors citation comme entre guillemets', () => {
    ...
    expect(messageSafe(new Error(`lecture de ${REFERENCE} refusée`))).toBe(
      'lecture de op://… refusée',
    )
    expect(messageSafe(new Error(`valeur "${REFERENCE}" refusée`))).toBe('valeur "op://…" refusée')
    expect(messageSafe(new Error(`could not read secret '${REFERENCE}'`))).toBe(
  ...
  it('caviarde une référence en fin de chaîne, avec ou sans point', () => {
    ...
    expect(messageSafe(new Error(`lecture de ${REFERENCE}`))).toBe('lecture de op://…')
    expect(messageSafe(new Error(`lecture de ${REFERENCE}.`))).toBe('lecture de op://….')
  })
  ...
  it('caviarde deux références dont l’une est le préfixe de l’autre', () => {
    ...
    expect(
      messageSafe(
        new Error(`${REFERENCE} et op://Coffre de démonstration/Fiche imaginaire échouent`),
      ),
  ...
  it('caviarde une référence qui ne nomme qu’un coffre', () => {
    ...
    expect(messageSafe(new Error('lecture de op://Coffre de démonstration refusée'))).toBe(
      'lecture de op://… refusée',
    )
  })
  ...
  it('ne retire pas le corps d’une référence quand le préfixe n’y est pas', () => {
    ...
    expect(messageSafe(new Error(message))).toBe(message)
  })
  ...
  it('ne fait rien d’une variable qui ne porte que le préfixe', () => {
    ...
    expect(messageSafe(new Error(message))).toBe(message)
    expect(messageSafe(new Error('boum'))).toBe('boum')
  })
