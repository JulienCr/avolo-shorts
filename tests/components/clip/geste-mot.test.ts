...
const étendue = { start: 100, end: 200 }

describe('gesteSurMotBarré', () => {
  it('comble le trou quand le mot est dans l’étendue', () => {
    expect(gesteSurMotBarré(étendue, { start: 140, end: 140.4 })).toEqual({ kind: 'remonter' })
  })

  it('déplace la borne de début quand le mot est avant', () => {
    expect(gesteSurMotBarré(étendue, { start: 40, end: 40.3 })).toEqual({
      kind: 'borne',
      bord: 'start',
    })
  })
  ...
  it('déplace la borne de fin quand le mot est après', () => {
    expect(gesteSurMotBarré(étendue, { start: 320, end: 320.3 })).toEqual({
      kind: 'borne',
      bord: 'end',
    })
  })
  ...
  it('comble le trou quand le mot chevauche une borne', () => {
    ...
    expect(gesteSurMotBarré(étendue, { start: 99.8, end: 100.2 })).toEqual({ kind: 'remonter' })
    expect(gesteSurMotBarré(étendue, { start: 199.8, end: 200.2 })).toEqual({ kind: 'remonter' })
  })
  ...
  it('remonte, faute d’étendue, quand tous les mots ont été retirés', () => {
    ...
    expect(gesteSurMotBarré(null, { start: 40, end: 40.3 })).toEqual({ kind: 'remonter' })
  })
})
